...
public class WeightLogActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        loadBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                pDialog = new SweetAlertDialog(  WeightLogActivity.this, SweetAlertDialog.PROGRESS_TYPE);
    ...
    // Method for fetching existing Logs from WeiLogs
    private void getExistingLogs() {
        FirebaseFirestore.getInstance().collection("WeiLogs").document(FirebaseAuth.getInstance().getCurrentUser().getUid()).get().addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
            @Override
            public void onSuccess(DocumentSnapshot documentSnapshot) {
                pDialog.dismissWithAnimation();
                wTv.setText("Weight : "+documentSnapshot.getString("weight")+"kg");
                hTv.setText("Height : "+documentSnapshot.getString("height"));
            ...
            public void onFailure(@NonNull Exception e) {
                pDialog.dismissWithAnimation();
                Toast.makeText(WeightLogActivity.this, ""+e.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void validateData() {
        ...
        if (wEt.getText().toString().length()<1){
            wEt.setError("This field must be filled");
            return;
        }
    ...
    private void saveData() {
        ...
        Map wieghtMAp=new HashMap();
          wieghtMAp.put("weight",wEt.getText().toString());
          wieghtMAp.put("height",hEt.getText().toString());
          wieghtMAp.put("date",dEt.getText().toString());
        FirebaseFirestore.getInstance().collection("WeiLogs").document(FirebaseAuth.getInstance().getCurrentUser().getUid()).set(wieghtMAp).addOnSuccessListener(new OnSuccessListener<Void>() {
            @Override
            public void onSuccess(Void aVoid) {


                pDialog.setTitleText("Congratulations !")
                        .setContentText("Your logs has been updated successfully.").setConfirmButton("Ok", new SweetAlertDialog.OnSweetClickListener() {
                    @Override
    ...
    private void showDatePickerDialogue() {
        ...
        DatePickerDialog datePickerDialog = new DatePickerDialog(WeightLogActivity.this, android.R.style.Theme_DeviceDefault_Dialog, mDatesetListener, year, month, day);
